...
void B1EventAction::EndOfEventAction(const G4Event*)
{
       ...
       Ken > 0 && !TMath::IsNaN(th) )
  {
    ...
    fRunAction->MuonEloss.push_back(fMuonEloss);
    fRunAction->Mom.push_back(mom);
    fRunAction->Momx.push_back(momx);
    fRunAction->Momy.push_back(momy);
    fRunAction->Momz.push_back(momz);

    // if(hit1 && hit2 && hit3)
    // {
    // G4cout << "Triple Coincidence!" << G4endl;
    // }
...
void B1EventAction::AddEdep1(G4double edep)
{
  fEdep1 += edep;
  // if (edep > 0.) hit1 = true;
}

void B1EventAction::AddEdep2(G4double edep)
{
  fEdep2 += edep;
  // if (edep > 0.) hit2 = true;
}

void B1EventAction::AddEdep3(G4double edep)
{
  fEdep3 += edep;
  // if (edep > 0.) hit3 = true;
}

void B1EventAction::AddMuonEloss(G4double eloss)
{
  fMuonEloss += eloss;
}
