...
void DMXSteppingAction::UserSteppingAction(const G4Step *fStep)
{
  ...
  // G4cout << "volume: " << volume << " --> fScoringVolumeLab : " << fScoringVolumeLab << " fScoringVolumeCav : " << fScoringVolumeCav << " fScoringVolumeFlo: " << fScoringVolumeFlo << " fScoringVolumePMT : " <<fScoringVolumePMT << G4endl;

  // G4cout << "DEBUG: Step in volume: " << volume->GetName() << G4endl;
  ...
  // G4cout << "Step in volume: " << volume->GetName()
  //      << " | Particle: " << atrack->GetDefinition()->GetParticleName()
  //      << " | KE: " << atrack->GetKineticEnergy()/GeV << " GeV"
  //      << G4endl;
  ...
  if (atrack->GetDefinition() == G4Neutron::NeutronDefinition())
  {
    ...
    // G4cout << "I am storing only " << pID << G4endl;
