...
for i in $(seq 1 1 $nWant); do 
    ...
    runid=$(( $runid + $b ))
    DESTINATION="/home/bsms.1/Monalisa/GridOutput/depth_815m"
    ...
    ### submission command
    qsub -l mem=32gb,nodes=1:ppn=6,walltime=120:00:00 -v parname1=${runid},parname2=${PRESENTDIRECTORY} -q medium -N "my_${runid}" -o "${DESTINATION}/run_$runid/my_$runid.out" -e "${DESTINATION}/run_$runid/my_$runid.err" runClusterScript.sh
