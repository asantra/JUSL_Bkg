...
DMXSteppingAction::DMXSteppingAction(DMXRunAction *DMXRun, DMXEventAction *eventAction)
    : runAction(DMXRun), evtAction(eventAction), fScoringVolumeLab(0),
      fScoringVolumePMT(0), fScoringVolumeCav(0), fScoringVolumeFlo(0), fScoringVolumeGlass(0), fScoringVolumeAir(0),
      fScoringVolumeHDPE(0)
{
  pTrackID = 0;
  pTrackID_pmt = 0;
...
void DMXSteppingAction::UserSteppingAction(const G4Step *fStep)
{
  ...
  // if (!fScoringVolumeSteel)
  // {
  //   const DMXDetectorConstruction *detectorConstruction = static_cast<const DMXDetectorConstruction *>(G4RunManager::GetRunManager()->GetUserDetectorConstruction());

  //   fScoringVolumeSteel = detectorConstruction->GetScoringVolumeSteel();
  // }

  if (!fScoringVolumeHDPE)
  {
    const DMXDetectorConstruction *detectorConstruction = static_cast<const DMXDetectorConstruction *>(G4RunManager::GetRunManager()->GetUserDetectorConstruction());

    fScoringVolumeHDPE = detectorConstruction->GetScoringVolumeHDPE();
  }

  if (!fScoringVolumeAir)
  {
    const DMXDetectorConstruction *detectorConstruction = static_cast<const DMXDetectorConstruction *>(G4RunManager::GetRunManager()->GetUserDetectorConstruction());

    fScoringVolumeAir = detectorConstruction->GetScoringVolumeAir();
  }
  ...
  if (//volume != fScoringVolumeLab &&
      ...
      // volume != fScoringVolumeSteel &&
      volume != fScoringVolumeHDPE &&
      volume != fScoringVolumeAir &&
  ...
  if (atrack->GetDefinition() == G4Neutron::NeutronDefinition())
  {
    ...
    if (evtAction->flag_air1 && evtAction->flag_hdpe && evtAction->flag_air2)
    {
    ...
    if (evtAction->flag_det1 && evtAction->flag_dhdpe && evtAction->flag_det2)
    {
      evtAction->flag_dhdpe = false;
      // evtAction->flag_dhdpe = false;
      evtAction->flag_det2 = false;
    ...
    if (volume == fScoringVolumeAir &&
    fStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)
    {
      evtAction->flag_air1 = true;
      ...
      evtAction->elvx.push_back(vx);
      evtAction->elvy.push_back(vy);
    ...
    fStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)
    {
      ...
      if (evtAction->detcount == 0)
      {
        ...
        evtAction->edvx.push_back(vx);
        evtAction->edvy.push_back(vy);
      }
      evtAction->detcount++;
      evtAction->Endet = atrack->GetKineticEnergy() / MeV;
    }

    if (volume == fScoringVolumeHDPE)
    {
      if (evtAction->flag_air1)
        evtAction->flag_hdpe = true;

      if (evtAction->flag_det1)
        evtAction->flag_dhdpe = true;
    }

    if (evtAction->flag_hdpe && volume == fScoringVolumeAir &&
    fStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)
    {
      ...
      evtAction->elBvx.push_back(vx);
      evtAction->elBvy.push_back(vy);
    ...
    if (evtAction->flag_dhdpe && volume == fScoringVolumePMT &&
    fStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)
    {
      ...
      evtAction->edBvx.push_back(vx);
      evtAction->edBvy.push_back(vy);
    }
