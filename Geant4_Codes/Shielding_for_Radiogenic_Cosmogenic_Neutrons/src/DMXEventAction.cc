...
void DMXEventAction::EndOfEventAction(const G4Event *evt)
{
  ...
  for (int i = 0; i < eleID.size(); i++)
  {
    ...
    runAct->rlvx.push_back(elvx[i]);
    runAct->rlvy.push_back(elvy[i]);
  }
  ...
  for (int i = 0; i < elBeID.size(); i++)
  {
    ...
    runAct->rlBvx.push_back(elBvx[i]);
    runAct->rlBvy.push_back(elBvy[i]);
  }
  ...
  for (int i = 0; i < edeID.size(); i++)
  {
    ...
    runAct->rdvx.push_back(edvx[i]);
    runAct->rdvy.push_back(edvy[i]);
  }
  ...
  for (int i = 0; i < edBeID.size(); i++)
  {
    ...
    runAct->rdBvx.push_back(edBvx[i]);
    runAct->rdBvy.push_back(edBvy[i]);
  }
