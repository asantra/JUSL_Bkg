...
//		 std::ios::out | std::ios::trunc);
  ...
  NeutronEnTr.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/TotalNeutrons/hdpe_shield/total_neutron_shielding/depth_815m/5cm_hdpe_shield/NeutronTrSpec.dat",
                   std::ios::out | std::fstream::app);
  NeutronEnDet.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/TotalNeutrons/hdpe_shield/total_neutron_shielding/depth_815m/5cm_hdpe_shield/NeutronDetSpec.dat",
                    std::ios::out | std::fstream::app);
  NeutronBack.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/TotalNeutrons/hdpe_shield/total_neutron_shielding/depth_815m/5cm_hdpe_shield/NeutronBackScatter.dat",
                   std::ios::out | std::fstream::app);
  NeutronDetBack.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/TotalNeutrons/hdpe_shield/total_neutron_shielding/depth_815m/5cm_hdpe_shield/DetBackScatter.dat",
                   std::ios::out | std::fstream::app);
...
void DMXRunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  for(int i=0;i<rlBeID.size();i++)
  {
      NeutronBack << rlBeID[i] << "\t" << rlBpID[i] << "\t" << rlBtID[i] << "\t"
                  ...
                  << rlBvz[i] << "\t" << rlBvx[i] << "\t" << rlBvy[i] << G4endl;
  }
  ...
  for(int i=0;i<rdBeID.size();i++)
  {
      NeutronDetBack << rdBeID[i] << "\t" << rdBpID[i] << "\t" << rdBtID[i] << "\t"
                     ...
                     << rdBvz[i] << "\t" << rdBvx[i] << "\t" << rdBvy[i] << G4endl;
  }
