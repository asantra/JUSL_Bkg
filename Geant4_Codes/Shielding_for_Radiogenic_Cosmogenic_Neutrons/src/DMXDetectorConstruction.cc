...
//*******************************AIR GAP INSIDE THE INSIDE THE HDPE SHIELDING FOR THE LAB******************************** */

// G4double air_halfX = 120.*cm;
// G4double air_halfY = 120.*cm;
// G4double air_halfZ = 60.*cm;
G4double air_halfX = 105.*cm;
G4double air_halfY = 105.*cm;
G4double air_halfZ = 45.*cm;

//********************** HDPE/POLYETHYLENE SHIELDING ******************* //

G4double hdpe_thickness = 5.0*cm;

// HDPE shield outer dimensions (BOX)
G4double hdpe_halfX = air_halfX + hdpe_thickness;     
G4double hdpe_halfY = air_halfY + hdpe_thickness;
G4double hdpe_halfZ = air_halfZ + hdpe_thickness;

G4ThreeVector hdpe_pos = G4ThreeVector(0.*cm , 0.*cm , 65.*cm);

// HDPE shield solid
G4Box* hdpe_solid = new G4Box("HDPEShield",
                            hdpe_halfX, hdpe_halfY, hdpe_halfZ);

hdpe_log = new G4LogicalVolume(hdpe_solid, hdpe_mat, "hdpe_log");
hdpe_phys = new G4PVPlacement(0,                                      //
       hdpe_pos,
       "hdpe_phys",
       hdpe_log,
       lab_phys,
...
// Visualisation
G4VisAttributes* hdpe_vat = new G4VisAttributes(magenta);
hdpe_vat->SetForceSolid(false);
hdpe_vat->SetVisibility(true);
hdpe_log->SetVisAttributes(hdpe_vat);
fScoringVolumeHDPE = hdpe_log;

 // ********************* Air Inside the Steel/HDPE Box **********************************************


//   G4double air_halfX = pmt_outerRadius;
//   G4double air_halfY = pmt_outerRadius;
//   G4double air_halfZ = pmt_halfz;
...
// Visualisation
G4VisAttributes* air_vat = new G4VisAttributes(cyan);
air_vat->SetForceSolid(false);
air_vat->SetVisibility(true);
air_log->SetVisAttributes(air_vat);
fScoringVolumeAir = air_log;
...
G4ThreeVector steelbx_pos = G4ThreeVector(0.*cm , 0.*cm , 60.*cm);
...
// G4double steel_thickness = 5.0*cm;

// //   // PMT dimensions (already defined)
// // G4double pmt_radius = pmt_outerRadius;
// // G4double pmt_halfz  = pmt_halfz;

// // steel shield outer dimensions (BOX)
// G4double steel_halfX = air_halfX + steel_thickness;     
// G4double steel_halfY = air_halfY + steel_thickness;
// G4double steel_halfZ = air_halfZ + steel_thickness;

// // Steel shield solid
// G4Box* steel_solid = new G4Box("SteelShield",
//                             steel_halfX, steel_halfY, steel_halfZ);

// steel_log = new G4LogicalVolume(steel_solid, steel_mat, "steel_log");
// steel_phys = new G4PVPlacement(0,                                      //
//        steelbx_pos,
//        "steel_phys",
//        steel_log,
//        lab_phys,
...
// // Visualisation
// G4VisAttributes* steel_vat = new G4VisAttributes(grey);
// steel_vat->SetForceSolid(false);
// steel_vat->SetVisibility(true);
// steel_log->SetVisAttributes(steel_vat);
// fScoringVolumeSteel = steel_log;
...
// // HDPE shield outer dimensions (BOX)
// G4double hdpe_halfX = glass_outerRadius + hdpe_thickness;     
// G4double hdpe_halfY = glass_outerRadius + hdpe_thickness;
// G4double hdpe_halfZ = glass_halfz + hdpe_thickness;
...
hdpe_log->SetUserLimits(theUserLimitsForRoom);
return world_phys;
}
