...
TFile *fIn = new TFile("/home/slab/Monalisa/JUSL_Bkg/MakingPlots/NeutronHistogramPlots/preliminaryHistograms_Neutrons_Nominal_DETCATII_1_1_26_DETCATII_withoutErrorBars.root", "READ");  //change the path for 555m and 815m depth accordingly.
...
void DMXPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
 ...
 G4double steel_center_z = 60.0*cm;


 x = -steel_halfX;          // -X face
 y = (G4UniformRand() - 0.5) * 2.0 * steel_halfY;
 z = steel_center_z + (G4UniformRand() - 0.5) * 2.0 * steel_halfZ;
 ...
 if (true)
 {
   ...
   dir = G4ThreeVector(1, 0, 0); // define the direction
