...
//    G4cout << "File Unreadable" << G4endl;
 ...
 energySpectrum.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/Neutrons/Radiogen/DataFiles/MTFiles/Hemisphere/2.5mThick/SiO2_norm/LowDen/Comp1/hdpe_shield/NeuSpec_generated.dat",
                     std::ios::out | std::fstream::app);
 particlePosition.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/Neutrons/Radiogen/DataFiles/MTFiles/Hemisphere/2.5mThick/SiO2_norm/LowDen/Comp1/hdpe_shield/ParticlePosition_generated.dat",
                       std::ios::out | std::fstream::app);
 particleDirection.open("/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/Neutrons/Radiogen/DataFiles/MTFiles/Hemisphere/2.5mThick/SiO2_norm/LowDen/Comp1/hdpe_shield/ParticleDirection_generated.dat",
                        std::ios::out | std::fstream::app);
 i = 0;
 pid = 2112;
}
...
TFile *fIn = new TFile("/home/slab/Monalisa/JUSL_Bkg/MakingPlots/CosmNeutronFlux/Cosmogenic_555m_LabCatII_13_2_26_LABCATII_COMBINED.dat.root", "READ");  //change the path for 555m and 815m depth accordingly.
TH1D *hEnergy = (TH1D*)fIn->Get("histLinearEnergy");

// /// @brief  wrong one
// TFile *fIn = new TFile("/home/slab/Monalisa/JUSL_Bkg/MakingPlots/CosmNeutronFlux/CosmogenicNeutron_WRONG_BINNING_13_2_26_labCATII_WithoutErrorBars_LABCATII_COMBINED.dat.root", "READ");  //change the path for 555m and 815m depth accordingly.
// TH1D *hEnergy = (TH1D*)fIn->Get("histLinearEnergy");




// TFile *fIn = new TFile("/home/slab/Monalisa/JUSL_Bkg/MakingPlots/NeutronHistogramPlots/preliminaryHistograms_Neutrons_Nominal_LABCATII_1_1_26_lab_withoutErrorBars.root", "READ");  //change the path for 555m and 815m depth accordingly.
// TH1D *hEnergy = (TH1D*)fIn->Get("histNeutronEnergy");
...
void DMXPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
 ...
 // thickness = 5.0 ;
 // --- air dimensions ---
 G4double air1_halfX = 25.0*cm;
 G4double air1_halfY = 25.0*cm;
//  G4double air1_halfZ = 35.0*cm;


 G4double air1_z = 190.5*cm;


 x = (G4UniformRand() - 0.5) * 2.0 * air1_halfX;          // +X face
 y = (G4UniformRand() - 0.5) * 2.0 * air1_halfY;
 z = air1_z;
 ...
 energy = hEnergy->GetRandom();
//  G4cout << "My energy: " << energy << G4endl;
  // phi = G4UniformRand() * 2.0 * M_PI;  // rpos->Rndm()*2.0*M_PI ;
 ...
 if (true)
 {
   particlePosition << pid << '\t' << x/cm << '\t' << y/cm << '\t' << z/cm << G4endl;


   dir = G4ThreeVector(0, 0, -1); // define the direction


   particleDirection << pid << '\t' << dir.x() << '\t'
                     << dir.y() << '\t'
                     << dir.z() << G4endl;;
   i++;
 }
