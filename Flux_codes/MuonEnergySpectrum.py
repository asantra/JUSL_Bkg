...
def main():
   ...
   parser.add_argument('-f', type=str, default="Muons_554m_combined_UP.dat")        #FOR 815M DEPTH: GeantGeneratedMuons_814m_combined.dat
   parser.add_argument('-t', type=str, default="primary")
   args = parser.parse_args()


   ### output root file
   outRoot = TFile("preliminaryHistograms_Muons_Trial_"+args.t+".root","RECREATE")
   outRoot.cd()
   histEnergy = TH1D("histEnergy", "Energy vs Flux; Energy (GeV); Flux (muons/cm^{2}/s/GeV)", 1000, 0, 10000)  # no title at all
   histEnergy.GetXaxis().SetTitle("Energy (GeV)")
   histEnergy.GetYaxis().SetTitle("Flux (muons/cm^{2}/s/GeV)")

   ### folder where the input files live
   inputfolder = "/home/slab/Monalisa/ClusterOutput/555m_systematics/density_up"
#    inputfolder = "/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/Muons_815m/815mData/NewRock/SiO2_norm"        # FOR 815M DEPTH 
   ...
   with open(fullFileName) as inFile:
       for lines in inFile.readlines():
           ...
           if args.t=="primary":
               ...
               theta       = float(eachLine[2])  
               #theta_deg = math.degrees(theta)
               #histAngle.Fill(theta)
   ...
   T_sec = 285000 / (A * exp_flux)  # seconds            #600000   #1335000
   ...
   for bin_idx in range(1, histEnergy.GetNbinsX() + 1):
    ...
    #    histEnergy.SetBinError(bin_idx, delta_flux)
       ...
       print(f"Bin {bin_idx}: N = {N}, deltaE = {deltaE}, deltaOmega = {deltaOmega}, flux = {flux}, T_sec = {T_sec}")
