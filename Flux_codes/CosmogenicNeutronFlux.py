# CosmNeutronTrSpec_combined.dat = Cosmogenic neutrons without the backscattered ones in the lab 
# CosmLabSpec_combined.dat = Cosmogenic neutrons including the backscattered ones in the lab 
# CosmNeutronDetSpec_combined.dat = Cosmogenic neutrons without the backscattered ones on the detector
# CosmDetSpec_combined.dat = Cosmogenic neutrons including the backscattered ones on the detector
...
def main():
   ...
   parser.add_argument('-f', type=str, default="LABCATII_COMBINED.dat")  # change the .dat files accoridngly
   parser.add_argument('-t', type=str, default="lab")                  # lab or detector
   args = parser.parse_args()

   ### output root file
   outputFolder = "/home/slab/Monalisa/JUSL_Bkg/MakingPlots/CosmNeutronFlux"
   outRoot = TFile(outputFolder + "/Cosmogenic_555m_LabCatII_13_2_26_"+args.f+".root", "RECREATE")    # change the name accordingly
   outRoot.cd()
   ...
   print("log_edges: ", len(log_edges), " ", log_edges)

    # Create the histogram with variable bins
   histLogEnergy = ROOT.TH1D("histLogEnergy", 
                           "Energy vs Flux; Energy (MeV); Flux (neutrons/cm^{2}/s/GeV)",
                      nbins, log_edges)

   #    )
   histLinearEnergy =TH1D("histLinearEnergy", "Energy vs Flux; Energy (GeV); Flux (neutrons/cm^{2}/s/GeV)", 1000, 0, 100)
...
#    histEnergy =TH1D("histEnergy", "Energy vs Flux; Energy (GeV); Flux (muons/cm^{2}/s/MeV)", 1000, 0, 10000)
   ...
   inputfolder = "/home/slab/Monalisa/JUSL_Bkg/JUSLFiles/CosmNeut/Cosmogen/DataFiles/MTFiles/Hemisphere/2.5mThick/SiO2_norm/LowDen/Comp1/depth_555m/Runs for final Measurement"          # OR depth_815m
   ...
   total_flux_root = histLogEnergy.Integral(0, histLogEnergy.GetNbinsX()+1,"width")
   ...
   print(f"\nFinal integrated flux (log) = ({flux_val:.2f}) * 1e-8 cm⁻² s⁻¹") 


   for bin_idx in range(0, histLinearEnergy.GetNbinsX() + 1 + 1):
   ...
   total_flux_root = histLinearEnergy.Integral(0, histLinearEnergy.GetNbinsX()+1,"width")
   ...
   # flux_err = total_uncertainty *1e8
   print(f"\nFinal integrated flux (linear) = ({flux_val:.2f}) * 1e-8 cm⁻² s⁻¹") 
